...
const familyCreationSchema = z.object({
  family: z.object({
    name: z.string().min(2, 'Family name must be at least 2 characters').max(100, 'Family name must be at most 100 characters'),
  ...
  children: z.array(z.object({
    name: z.string().min(2, 'Child name must be at least 2 characters').max(50, 'Child name must be at most 50 characters'),
    ...
    created_by: z.string().uuid('Invalid user ID')
  })).min(1, 'At least one child is required').max(10, 'Maximum 10 children allowed')
})
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (membershipCheckError) {
      console.error('Error checking existing memberships:', membershipCheckError)
    ...
    if (childrenError) {
      ...
      await adminClient.from('family_memberships').delete().eq('family_id', family.id)
      await adminClient.from('families').delete().eq('id', family.id)
      return NextResponse.json(

...
async function performSemanticSearch(params: SearchQuery): Promise<{
  ...
  try {
    ...
    const searchResults = await callRPCWithFallback('search_memories_semantic', {
    ...
    const countResult = await callRPCWithFallback('count_semantic_search_results', {
      query_text: params.query,
      query_embedding: queryEmbedding,
      family_id: params.familyId,
      similarity_threshold: params.similarity_threshold,
    ...
    const totalCount = Array.isArray(countResult) && countResult.length > 0 ? (countResult[0]?.count || 0) : memories.length

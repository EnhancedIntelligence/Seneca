    ...
    return NextResponse.json({
      ...
      data: {
        ...
        memories: memories.map((memory) => ({
          ...
          child_name:
            (memory as any).children?.name || (memory as any).child?.name,
          family_name:
            (memory as any).families?.name || (memory as any).family?.name,
      ...
      return NextResponse.json(
        {
          ...
          error: error.message,
          suggestion:
            "Make sure test family and child exist first by running the SQL script",
        },
        { status: 500 },
      );
    }

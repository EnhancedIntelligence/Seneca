...
const MENU_ITEMS: MenuItem[] = [
  { id: 'profile', label: 'My Profile', icon: User, description: 'View and edit your profile' },
  ...
  { id: 'privacy', label: 'Privacy & Security', icon: Shield, description: 'Privacy settings' },
  { id: 'help', label: 'Help & Support', icon: HelpCircle, description: 'Get help', divider: true },
...
export function ProfileMenu({
  ...
  const getPlanBadge = () => {
    ...
    const planStyles = {
      ...
      enterprise: 'bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-400'
    }
    
    return (
      <span className={cn('px-2 py-0.5 rounded-full text-xs font-medium', planStyles[user.plan])}>
  ...
  const getPosition = () => {
    if (anchorEl) {
      ...
      return {
        top: rect.bottom + 8,
        right: position === 'right' ? window.innerWidth - rect.right : undefined,
    ...
    >
      ...
      <div className="p-3 border-t border-white/10 text-center">
        <p className="text-xs text-white/40">
          Version 1.0.0 • <a href="#" className="hover:text-white/60 transition-colors">Terms</a> • <a href="#" className="hover:text-white/60 transition-colors">Privacy</a>
        </p>
      </div>
    </div>
  )

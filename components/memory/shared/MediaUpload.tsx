...
export function MediaUpload({
  ...
  const handleFileSelect = useCallback(async (files: FileList | File[]) => {
    ...
    // Upload files
    const uploadPromises = newUploads.map(uploadItem => uploadFile(uploadItem))
  ...
  const uploadFile = async (uploadItem: UploadingFile): Promise<void> => {
    try {
      ...
      const completedCount = uploadingFiles.filter(f => f.status === 'completed').length + 1
  ...
  return (
    <div className={className}>
      ...
      {value.length > 0 && (
        <div className="mt-4 space-y-2">
          <h4 className="text-sm font-medium">Uploaded Files ({value.length}/{maxFiles})</h4>

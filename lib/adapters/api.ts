...
export function apiChildToUi(child: any): UIChild {
  ...
  return {
    ...
    name: child.name,
    avatarUrl: child.avatarUrl || child.profileImageUrl || child.profile_image_url || null,
...
export function apiMemoryToUi(memory: any): UIMemory {
  ...
  const deriveType = () => {
    ...
    if (memory.videoUrls?.length > 0 || memory.video_urls?.length > 0) return 'video';
    if (memory.imageUrls?.length > 0 || memory.image_urls?.length > 0) return 'photo';
  ...
  return {
    ...
    timestamp: memory.timestamp || memory.memoryDate || memory.memory_date || memory.createdAt || memory.created_at || new Date().toISOString(),
    type: deriveType(),
    tags: normalizeTags(memory.tags),
    category: memory.category || null,
    needsReview: memory.needsReview ?? memory.needs_review ?? false,
    processingStatus: memory.processingStatus || memory.processing_status || 'pending',
    ...
    ...(has(memory.locationName) ? { locationName: memory.locationName } : 
        has(memory.location_name) ? { locationName: memory.location_name } : {}),
    ...(has(memory.locationLat) ? { locationLat: memory.locationLat } :
        has(memory.location_lat) ? { locationLat: memory.location_lat } : {}),
    ...(has(memory.locationLng) ? { locationLng: memory.locationLng } :
        has(memory.location_lng) ? { locationLng: memory.location_lng } : {}),
    ...(has(memory.milestoneDetected) ? { milestoneDetected: memory.milestoneDetected } :
        has(memory.milestone_detected) ? { milestoneDetected: memory.milestone_detected } : {}),
    ...(has(memory.milestoneType) ? { milestoneType: memory.milestoneType } :
        has(memory.milestone_type) ? { milestoneType: memory.milestone_type } : {}),
    ...(has(memory.milestoneConfidence) ? { milestoneConfidence: memory.milestoneConfidence } :
        has(memory.milestone_confidence) ? { milestoneConfidence: memory.milestone_confidence } : {}),
    ...(has(memory.memoryDate) ? { memoryDate: memory.memoryDate } :
        has(memory.memory_date) ? { memoryDate: memory.memory_date } : {}),
  };
}

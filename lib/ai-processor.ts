...
class AIProcessor {
  ...
  async processMemory(memoryId: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Successfully processed memory ${memoryId} in ${
  ...
  private async getMemoryContext(
    ...
    if (error || !memory) {
      throw new ProcessingError(`Memory not found: ${memoryId}`, {
        memoryId,
        error,
      });
    }
    ...
    const childInfo = child
      ? {
          ...
          ageInMonths: child.birth_date
            ? this.calculateAgeInMonths(child.birth_date)
            : null,
        }
      : null;
...
`;
  ...
  private async logProcessingAnalytics(
    ...
    try {
      ...
      await this.adminClient.from("processing_analytics").insert({
        ...
        tokens_used: result.processingCost
          ? Math.round(result.processingCost * 33333)
          : null, // Rough estimate
  ...
  private simulateAIProcessing(
    ...
    if (content.includes("birthday")) category = "birthday";
      ...
      content.includes("christmas") ||
      content.includes("halloween")
    )
      category = "holiday";
    else if (content.includes("first") || content.includes("learned"))
      category = "achievement";
    else if (content.includes("school")) category = "education";
    else if (content.includes("play")) category = "social_interaction";
    else if (content.includes("creative") || content.includes("draw"))
    ...
    return {
      ...
      summary: `A ${sentiment} family memory about ${category.replace(
        "_",
        " "
      )}`,
      ageAtMemory: context.child?.birth_date
        ? this.calculateAgeInMonths(context.child.birth_date) ?? undefined
        : undefined,
      ...
      emotions:
        sentiment === "positive"
          ...
          ? ["frustration", "sadness"]
          : ["calm", "neutral"],
      developmentalStage: context.child
        ? "Age-appropriate development"
        : "General family bonding",
    };
  }

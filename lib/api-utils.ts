...
export function validateQueryParams(request: NextRequest, schema: any): any {
...
export function withApiHandler<T extends any[], R>(
  ...
  return async (...args: T): Promise<NextResponse> => {
    ...
    try {
      ...
      response.headers.set(
        "X-Response-Time",
        monitor.getMetrics().execution_time.toString(),
      );
      response.headers.set(
        "X-Queries-Count",
        monitor.getMetrics().queries_count.toString(),
      );

...
export class BackgroundWorker {
  ...
  private async processJobConcurrently(job: ProcessingJob): Promise<void> {
    ...
    try {
      console.log(`📝 Processing job ${jobId} for memory ${job.payload.memoryId}`)

      // Set up timeout for job processing
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(new ProcessingError(`Job ${jobId} timed out after ${this.config.processingTimeout}ms`))
  ...
  private async waitForJobsToComplete(): Promise<void> {
    ...
    while (this.currentJobs.size > 0) {
      if (Date.now() - startTime > this.config.shutdownTimeout) {
        console.warn(`⏰ Shutdown timeout reached, forcibly stopping ${this.currentJobs.size} jobs`)
        break
      }
...
export function createBackgroundWorker(config?: Partial<WorkerConfig>): BackgroundWorker {
...
export class WorkerManager {
  ...
  async startWorker(workerId: string, config?: Partial<WorkerConfig>): Promise<BackgroundWorker> {

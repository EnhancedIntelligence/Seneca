...
function validateEnv() {
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      const requiredErrors = error.issues.filter(issue => 
...
export function logEnvStatus() {
  if (isDevelopment || isDebug) {
    ...
    console.log(`  - AI Features: ${aiConfig.isEnabled ? '✅ Enabled' : '⚠️ Disabled (no API key)'}`)
    console.log(`  - Rate Limiting: ${rateLimitConfig.isEnabled ? '✅ Enabled' : '⚠️ Disabled (no Upstash)'}`)

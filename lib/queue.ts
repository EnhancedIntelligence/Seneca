...
export class MemoryQueue {
  ...
  async processNextJob(): Promise<ProcessingJob | null> {
    try {
      ...
      const data = await callRPCWithFallback('get_next_job_and_lock', { worker_id: this.workerId })
  ...
  async retryFailedJob(jobId: string): Promise<boolean> {
    try {
      const data = await callRPCWithFallback('retry_failed_job', { job_id: jobId })
      return Boolean(data)
    } catch (error) {
  ...
  private calculateQueueHealth(stats: any): 'healthy' | 'degraded' | 'critical' {
  ...
  async getFailedJobs(limit = 50): Promise<any[]> {
    try {
      const data = await callRPCWithFallback('get_failed_jobs', { limit_count: limit })
      return data || []
    } catch (error) {

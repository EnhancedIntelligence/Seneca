...
export async function getUserFamilyIds(userId: string): Promise<string[]> {
  ...
  return (
    memberships
      ?.map((m: { family_id: string | null }) => m.family_id)
...
export async function requireSubscription(
  ...
  const expiresAtValue = member[SUBSCRIPTION_COLUMNS.expiresAt] as
    | string
    | null;
  ...
  if (allowedTiers.length > 0) {
    const userTier =
      (member[SUBSCRIPTION_COLUMNS.tier] as SubscriptionTier) ?? "free";

...
const serverEnv = (() => {
  try {
    ...
    if (coreConfig.app.nodeEnv === 'development') {
      console.log('ℹ️ Optional server features not configured (this is OK in dev)');
...
export const rateLimit = {
  isEnabled: !!(serverEnv.UPSTASH_REDIS_REST_URL && serverEnv.UPSTASH_REDIS_REST_TOKEN),
...
export const security = {
  ...
  corsOrigins: serverEnv.CORS_ORIGINS
    ? serverEnv.CORS_ORIGINS.split(',').map(o => o.trim()).filter(Boolean)
    : [],
...
export function logServerEnvStatus() {
  if (coreConfig.app.nodeEnv === 'development' || serverEnv.DEBUG) {
    ...
    console.log(`  - Supabase Admin: ${supabaseAdmin.hasServiceRole ? '✅' : '⚠️ No service role'}`);
    console.log(`  - AI Features: ${ai.isEnabled ? '✅' : '⚠️ Disabled'}`);
    console.log(`  - Rate Limiting: ${rateLimit.isEnabled ? '✅' : '⚠️ Disabled'}`);
    console.log(`  - Internal API: ${security.hasInternalApi ? '✅' : '⚠️ Disabled'}`);
    console.log(`  - Monitoring: ${monitoring.isEnabled ? '✅' : '⚠️ Disabled'}`);
  }
}
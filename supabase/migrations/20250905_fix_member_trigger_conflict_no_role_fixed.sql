...
=================================================
Triggers on auth.users could NOT be created automatically.
Please run: supabase/migrations/20250905_create_triggers_manual.sql
as role: supabase_auth_admin (via Supabase Dashboard)

To verify after manual creation, run:
SELECT tgname FROM pg_trigger t
JOIN pg_class c ON c.oid = t.tgrelid
JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE n.nspname=''auth'' AND c.relname=''users'' AND NOT t.tgisinternal;

IMPORTANT: The application will still work via the ensure_member() RPC
but triggers provide better reliability.
=================================================';
  END IF;
EXCEPTION
...
⚠️  MANUAL ACTION REQUIRED
=================================================
Insufficient privileges to create triggers on auth.users.
Please run: supabase/migrations/20250905_create_triggers_manual.sql
as role: supabase_auth_admin (via Supabase Dashboard)
...
=================================================
Functions created: %s of 3 expected
Triggers created: %s of 2 expected
Orphaned users: %s
Placeholder emails: %s
=================================================',
    func_count, trigger_count, orphan_count, placeholder_count);

  RAISE NOTICE '%', status_msg;

  IF trigger_count < 2 THEN
    RAISE WARNING '%', 
'⚠️  Triggers are missing. The application will work via ensure_member() RPC
but you should still create triggers manually for best reliability.

Run: supabase/migrations/20250905_create_triggers_manual.sql
